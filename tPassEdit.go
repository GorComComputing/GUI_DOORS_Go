...
func CreatePassEdit(parent *Node, name string, x int, y int, sizeX int, sizeY int, BC uint32, TC uint32, text string, onClick func(*Node), onEnter func(*Node)) *Node {
	obj := tPassEdit{name: name, x: x, y: y, sizeX: sizeX, sizeY: sizeY, BC: BC, TC: TC, text: text, visible: true, enabled: true, curX: 0, onClick: onClick, onEnter: onEnter}
	node := Node{typ: PASSEDIT, parent: parent, previous: nil, children: nil, obj: &obj}
	parent.children = append(parent.children, &node)
	return &node
}


func (obj *tPassEdit) Draw(parX int, parY int, parSizeX int, parSizeY int){
    ...
    var passStr string = ""
    for i := 0; i < len(obj.text); i++ {
    	passStr += "*"
    }
    
    if (len(obj.text)+1)*7 > obj.sizeX {
    	TextOutgl(nil, passStr, parX+obj.x + obj.sizeX - (len(obj.text)+1)*7, parY+obj.y + obj.sizeY/2-4, 1);
    } else {
    	TextOutgl(nil, passStr, parX+obj.x + 4, parY+obj.y + obj.sizeY/2-4, 1);
    }
...
func (obj *tPassEdit) RAD(x int, y int){
	var visible string
    		if obj.visible {
    			visible = "true"
    		} else {
...
func (obj *tPassEdit) KeyDown(key int){
	//fmt.Println("TEST")
...
func (obj *tPassEdit) Click(x int, y int){
...
func (obj *tPassEdit) MouseMove(x int, y int, Xl int, Yl int){
...
func (obj *tPassEdit) MouseDown(x int, y int){
	// RAD
